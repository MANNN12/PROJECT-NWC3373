...
# ==========================================
# SYSTEM TESTING HARNESS (MAIN RUNNER)
# ==========================================
if __name__ == "__main__":
    print("=== NWC3373 LFSR STREAM CIPHER ENGINE ===\n")
    
    # 1. Initialize the system with a shared secret key
    secret_key = "OurGroupSecretPassword"
    cipher = LFSRStreamCipher(key_string=secret_key)
    
    # 2. Generate a mock text file for demonstration (Approx. 1 KB sample)
    sample_input = "sample_data.txt"
    with open(sample_input, "w") as f:
        f.write("CONFIDENTIAL 0526: This is highly classified organizational data. " * 20)
        
    encrypted_output = "encrypted_file.bin"
    decrypted_output = "decrypted_file.txt"
    
    # 3. Execute ENCRYPTION PHASE (XOR Encryption Mechanism)
    print("[Phase 1] Initiating Encryption Process...")
    cipher.process_file(sample_input, encrypted_output)
    
    # 4. Execute DECRYPTION PHASE (A1. Successful Decryption)
    # Due to the symmetric involution property of XOR, running the identical 
    # keystream against the ciphertext restores the plaintext perfectly.
    print("[Phase 2] Initiating Decryption Process...")
    cipher.process_file(encrypted_output, decrypted_output)
    
    # 5. Output Verification (Demonstrate Correctness)
    with open(sample_input, 'r') as f1, open(decrypted_output, 'r') as f2:
        if f1.read() == f2.read():
            print("[Verification]: STATUS SUCCESSFUL! The decrypted file is a 100% exact match to the original source.")
        else:
            print("[Verification]: STATUS FAILED! File mismatch detected.")